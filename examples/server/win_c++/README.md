...
项目用Visual Studio 2015编译
依赖库为curl、openssl、jsoncpp，已经编译好静态库放在项目目录中
低版本VS可自行编译以上三个库进行依赖
...
1. 安装依赖<br>
SDK使用cURL库进行网络相关操作，JsonCpp库进行相关数据解析，如果您的系统还没有这些库，请先安装dep录下的两个库；
服务端在编译时增加链接选项：-lcrypto -ljsoncpp -lcurl
